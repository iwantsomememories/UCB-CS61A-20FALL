...
def prune_min(t):
    ...
    if Tree.is_leaf(t):
        return 
    left_son = t.branches[0]
    right_son = t.branches[1]
    if left_son.label <= right_son.label:
        prune_min(left_son)
        t.branches = [left_son]
    else:
        prune_min(right_son)
        t.branches = [right_son]
    return
...
def num_splits(s, d):
    ...
    def helper(index, sum1, sum2):
        if index == len(s):
            return 1 if abs(sum1 - sum2) <= d else 0

        count_with_first = helper(index+1, s[index]+sum1, sum2)
        count_with_second = helper(index+1, sum1, s[index]+sum2)

        return count_with_first + count_with_second
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            corrections = skel_char + edits
            possibilities.append((corrections, cost))
        # Insert
        edits, cost = helper_align(skeleton_idx, code_idx+1)
        corrections = "+[" + code_char + "]" + edits
        possibilities.append((corrections, cost+1))
        # Delete
...
def reverse(lst):
    ...
    True
    """
    return foldl(lst, lambda first, rest: Link(rest, first), Link.empty) 
