...
def swine_align(player_score, opponent_score):
    ...
    if player_score < 10 or opponent_score < 10:
        return False
    limit = min(player_score, opponent_score) 
    cur = 10
    while cur <= limit:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            cur_score = score1
        gap = cur_score - last_score
        if gap > running_high:
            print(f'{gap} point(s)! The most yet for Player {who}')
        return announce_highest(who, cur_score, max(gap, running_high))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_roll = 0
    averaged_func = make_averaged(roll_dice, trials_count)
    for i in range(1, 11):
        score = averaged_func(i, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if bacon_score >= cutoff:
        return 0
    if swine_align(bacon_score+score, opponent_score) or pig_pass(bacon_score+score, opponent_score):
        return 0
    return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    cutoff = GOAL_SCORE - score
    num_rolls = 5
    if extra_turn_strategy(score, opponent_score, cutoff, num_rolls) == 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 12
