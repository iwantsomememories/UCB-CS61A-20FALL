...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        print(f'DEBUG: {repr(expressions.rest)}')
        ...
        lambdaP = do_lambda_form(Pair(formals, body), env)

        name = target.first
        env.bindings[name] = lambdaP
        return name
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    body = expressions.rest
    lambdaP = LambdaProcedure(formals, body, env)
    return lambdaP
    # END PROBLEM 8
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        validate_form(definition, 2, 2)

        name = definition.first
        value = scheme_eval(definition.rest.first, env)
